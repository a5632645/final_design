...
void ThrianDispersion::Init(float sampleRate) {
    sampleRate_ = sampleRate;
}

float ThrianDispersion::Process(float in) {
...
float ThrianDispersion::GetPhaseDelay(float freq) const {
    auto omega = twopi * freq / sampleRate_;
    auto cpx = std::polar(1.0f, omega);
    auto up = b0_ * cpx * cpx + a1_ * cpx + b2_;
...
void ThrianDispersion::SetGroupDelay(float delay) {
    if (delay < 1.0f) {
        a1_ = 0.0f;
        a2_ = 0.0f;
        b0_ = 1.0f;
...
std::complex<float> ThrianDispersion::GetResponce(float omega) const {
...
float ThrianDispersion::ProcessFilter(float in, uint32_t i) {
