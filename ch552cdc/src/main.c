...
* Version            : V1.0
* Date               : 2017/03/01
* Description        : CH554 as CDC device to serial port, select serial port 0
*******************************************************************************/
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
void Config_Uart0(uint8_t *cfg_uart)
{
    ...
    // *((uint8_t *)&uart0_buad) = cfg_uart[0];
    // *((uint8_t *)&uart0_buad+1) = cfg_uart[1];
    // *((uint8_t *)&uart0_buad+2) = cfg_uart[2];
    // *((uint8_t *)&uart0_buad+3) = cfg_uart[3];
    // SBAUD = 256 - FREQ_SYS/16/uart0_buad; //  SBAUD1 = 256 - Fsys / 16 / baud rate
    ES = 1;
}
/*******************************************************************************
