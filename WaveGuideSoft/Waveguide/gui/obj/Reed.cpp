...
void CReed::Draw(StyleDrawer& drawer) {
    ...
    for (int8_t i = 0; i < totalKnobs_; ++i) {
        ...
        if (knobs_[i].GetParamDesc() == &dsp::SynthParams.reed.vibrateControl) {
            knobs_[i].DrawWTF(drawer.display, dsp::SynthParams.reed.vibrateControl, rect, dsp::kVibrateModeStrs);
        }
        else if (knobs_[i].GetParamDesc() == &dsp::SynthParams.reed.tremoloControl) {
            knobs_[i].DrawWTF(drawer.display, dsp::SynthParams.reed.tremoloControl, rect, dsp::kTremoloModeStrs);
        }
        else {
...
void CReed::SetPageIdx(int8_t page) {
    ...
    case ePage_page1:
        ...
        break;
    case ePage_page2:
        knobs_[0].BindParamDesc(params.reed.vibrateControl);
        knobs_[1].BindParamDesc(params.reed.vibrateRate);
        knobs_[2].BindParamDesc(params.reed.vibrateDepth);
        knobs_[3].BindParamDesc(params.reed.tremoloControl);
        knobs_[4].BindParamDesc(params.reed.tremoloRate);
        knobs_[5].BindParamDesc(params.reed.tremoloDepth);
        knobs_[6].BindParamDesc(params.reed.vibrateAttack);
        knobs_[7].BindParamDesc(params.reed.tremoloAttack);
        knobs_[8].BindParamDesc(params.reed.blendAdd);
        knobs_[9].BindParamDesc(params.reed.loopGainAdd);
        totalKnobs_ = 10;
        break;
    }
