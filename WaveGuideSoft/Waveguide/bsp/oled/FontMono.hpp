...
extern const font_header_t font_header;
extern const font_section_t font_sections[];
extern const font_index_t font_indexs[];
extern const uint8_t font_data[];

static constexpr uint32_t kInvalidFontIndexIndex = std::numeric_limits<uint32_t>::max();

inline static uint32_t GetFontIndexIndex(uint8_t code) {
    constexpr uint32_t kNumFontSections = 1;
    for (uint32_t i = 0; i < kNumFontSections; i++) {
        auto& section = font_sections[i];
        if (code >= section.first && code <= section.last) {
            return code - section.first + section.offset;
        }
    }
    return kInvalidFontIndexIndex;
}

inline static uint32_t GetFontIndexIndexSimpler(uint8_t code) {
    if (code >= font_sections[0].first && code <= font_sections[0].last) {
        return code - font_sections[0].first + font_sections[0].offset;
    }
    return kInvalidFontIndexIndex;
}
